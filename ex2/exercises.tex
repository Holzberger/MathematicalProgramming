...
Next show $P_{cec} \subseteq P_{sub}$.
First select an arbitrary $x \in P_{cec}$ which implies $\sum\limits_{e\in C}x_e\leq |C|-1\quad \forall |C|\geq 2 \text{ and } C\subseteq E$.

Consider an arbitrary cycle $C_1$. If we were to remove any edge $t$ from $C_1$ we end up with a path $C_1\setminus \{t\}$ that fulfills $|C_1|-1 = |C_1\setminus \{t\}|$. Therefore if $C_1$ satisfies the CEC $\sum\limits_{e\in C_1}x_e \leq |C_1|-1$ we have $\max\limits_{x_e}\sum\limits_{x_e\in C_1}x_e = |C_1|-1=|C_1\setminus \{t\}|=\max\limits_{x_e}\sum\limits_{x_e\in C_1\setminus \{t\}}x_e$.

Now take an arbitrary subtour $S$.
Any cycle of $S$ can be replaced by the path that we obtain by removing an edge from the corresponding cycle. By this we obtain a new acyclic graph $\hat{S}$ with $\max\limits_{x_e}\sum\limits_{x_e\in E(S)}x_e = \max\limits_{x_e}\sum\limits_{x_e\in E(\hat{S})}x_e$. In any acyclic graph the numver of vertices is at least one more than the number of edges. Therfore we have $\max\limits_{x_e}\sum\limits_{x_e\in E(\hat{S})}x_e = |E(\hat{S})|\leq |\hat{S}|-1$. Since $|S|=|\hat{S}|$ we further have $\max\limits_{x_e}\sum\limits_{x_e\in E(S)}x_e \leq |S|-1$ which shows that $\sum\limits_{x_e\in E(S)}x_e \leq |S|-1$ and we follow $x\in P_{sub}$. $\blacksquare$
